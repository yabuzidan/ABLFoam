...
const Foam::scalar Foam::mainABL::CmuDefault_ = 0.09;
...
:
    ...
    CmuMax_(readScalar(dict.lookup("CmuMax"))),
...
Foam::tmp<Foam::scalarField> Foam::mainABL::epsilon
(
    const vectorField& p
) const
{
    ...
    scalarField homCmu = min(pow(ustar_,4)/sqr(kref_),CmuMax_);

    
    tmp<scalarField> tepsilon
    (
        (sqrt(homCmu)*ustar_*kref_)/(kappa_*arg)
    );

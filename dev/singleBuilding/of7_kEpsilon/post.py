...
data2 = pd.read_csv('postProcessing/inletGraph/'+endTime+'/line_p_k_omega_nut.csv')

Z_inlet = data1['z']
U_inlet = data1['U_0']
k_inlet = data2['k']
omega_inlet = data2['omega']
nut_inlet = data2['nut']
p_inlet = data2['p']

# Load incident data
data1 = pd.read_csv('postProcessing/incidentGraph/'+endTime+'/line_U.csv')
data2 = pd.read_csv('postProcessing/incidentGraph/'+endTime+'/line_p_k_omega_nut.csv')

Z_incident = data1['z']
U_incident = data1['U_0']
k_incident = data2['k']
omega_incident = data2['omega']
...
axes[2].semilogx(omega_inlet, Z_inlet, 'k', marker = 'o', ms=3, lw=0.75)
axes[2].semilogx(omega_incident, Z_incident, 'r', marker='o', ms=3, lw=0.75)
if err:
    axes[2].text(0.70, 0.25, '$e$ = {}%'.format(calcError(omega_incident, omega_inlet)), color='k', transform=axes[2].transAxes)

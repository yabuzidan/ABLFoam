...
)
{
    ...
    forAll(nutw, facei)
    {
        ...
        const scalar uPlus = (1/kappa)*log(wallCoeffs_.E()*yPlus);
        ...
        if (blended_)
        {
            ...
            const scalar omegaLog = uStar/(betaStar5*kappa*y[facei]);
		...
		omega0[celli] += w*(1/(kappa*ustar_[facei]*(y[facei] + z0_[facei])))
				  *k[celli];
...
void omegaRoughABLWallFunctionFvPatchScalarField::write(Ostream& os) const
{
    ...
    os.writeEntry("WFbetaStar", WFbetaStar_);
